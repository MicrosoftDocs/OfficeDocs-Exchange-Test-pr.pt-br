---
title: 'Função ApplicationImpersonation: Exchange 2013 Help'
TOCTitle: Função ApplicationImpersonation
ms:assetid: d6e43a2f-9b1f-463e-8c56-ca38d9127a5a
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Dd776119(v=EXCHG.150)
ms:contentKeyID: 50486745
ms.date: 05/22/2018
...
_**Tópico modificado em:** 2016-11-30_

A função de gerenciamento `ApplicationImpersonation` permite que applicativos se passem por um usuário em uma organização para executar tarefas em nome do usuário.
...
> [!IMPORTANT]
> Um processo ou aplicativo que é membro da função <CODE>ApplicationImpersonation</CODE> pode acessar o conteúdo da caixa de correio de um usuário e atuar em nome desse usuário, mesmo se a conta de usuário estiver desabilitada. Isso pode permitir que os usuários acessem suas caixas de correio se você tiver aplicativos, como Blackberry Enterprise Server, que usam a função <CODE>ApplicationImpersonation</CODE>. Produtos de terceiros que não usam a função <CODE>ApplicationImpersonation</CODE>, mas sim o Exchange ActiveSync, não podem acessar uma caixa de correio após a conta de usuário ter sido desabilitada.<BR>Para impedir um aplicativo que usa a função <CODE>ApplicationImpersonation</CODE> de acessar uma caixa de correio ou realizar tarefas em seu nome após sua conta de usuário ter sido desabilitada, execute uma ou mais das seguintes ações: 
> <UL>
> <LI>
> <P>Desabilite ou remova o usuário no aplicativo de terceiros.</P>
> <LI>
> <P>Exclua a caixa de correio.</P></LI></UL>
