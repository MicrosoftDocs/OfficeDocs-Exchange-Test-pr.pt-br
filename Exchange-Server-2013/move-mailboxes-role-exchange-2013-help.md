---
title: 'Mover caixas de correio função: Exchange 2013 Help'
TOCTitle: Mover caixas de correio função
ms:assetid: 46576498-5d73-495b-8925-b8ce39cfa55e
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Dd876879(v=EXCHG.150)
ms:contentKeyID: 50485502
ms.date: 05/22/2018
...
A função de gerenciamento `Move Mailboxes` permite que os administradores movam caixas de correio entre servidores em uma organização e entre servidores na organização local e outra organização.
...
Além dos escopos de destinatário, o cmdlet **New-MoveRequest**, incluído nesta função, também é adicionado a um escopo por meio do uso de escopos de configuração de banco de dados. Os escopos de configuração de banco de dados controlam para quais bancos de dados o cmdlet pode mover as caixas de correio. O banco de dados para o qual você deseja mover uma caixa de correio deve estar no escopo de banco de dados. Essa condição se aplica quando você especifica um banco de dados utilizando o parâmetro *TargetDatabase* no cmdlet **New-MoveRequest** ou se você autorizar que a distribuição automática de caixas de correio selecione o banco de dados para você. Para mais informações, consulte [Noções básicas sobre escopos da função de gerenciamento](understanding-management-role-scopes-exchange-2013-help.md).
