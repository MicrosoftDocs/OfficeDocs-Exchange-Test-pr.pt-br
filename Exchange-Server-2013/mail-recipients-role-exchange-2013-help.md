---
title: 'Função de destinatários de email: Exchange 2013 Help'
TOCTitle: Função de destinatários de email
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50485993
ms.date: 05/22/2018
...
A função de gerenciamento de `Mail Recipients` permite que administradores gerenciem caixas de correio existentes, os usuários de email e contatos de email em uma organização. Essa função não pode criar esses destinatários. Use a função `Mail Recipient Creation` para criá-los.

Esse tipo de função não permite gerenciar pastas públicas habilitadas para email ou grupos de distribuição. Use as funções a seguir para gerenciar esses objetos:
...
Se sua organização tiver um modelo de permissões divididas onde a criação de destinatário e o gerenciamento são executadas por grupos diferentes, atribua a função `Mail Recipient Creation` ao grupo que executa a função `Mail Recipients` ao grupo que está executando o gerenciamento de destinatários e criação de destinatário. Para obter mais informações, consulte os tópicos a seguir:
...
Além dos escopos de destinatário, os cmdlets **Connect-Mailbox** e **Enable-Mailbox** , que são incluídos com esta função, também limitam usando escopos de configuração do banco de dados. Escopos de configuração do banco de dados controlam os cmdlets podem criar novas caixas de correio em que bancos de dados. O banco de dados onde você deseja criar uma caixa de correio deve estar dentro do escopo de banco de dados. Esta condição se aplica quando você especificar um banco de dados usando o parâmetro *Database* no cmdlet ou se você permitir a distribuição de caixa de correio automática selecionar o banco de dados para você. Para obter mais informações, consulte [Noções básicas sobre escopos da função de gerenciamento](understanding-management-role-scopes-exchange-2013-help.md).
