---
title: 'Função de isenção legal: Exchange 2013 Help'
TOCTitle: Função de isenção legal
ms:assetid: c98ce8ca-3477-479a-ad23-a8e6459bc4d0
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Dd876939(v=EXCHG.150)
ms:contentKeyID: 50486620
ms.date: 05/22/2018
...
A função de gerenciamento de `Legal Hold` permite aos administradores configurar se os dados dentro de uma caixa de correio devem ser retidos para fins de litígio em uma organização.
...
Além dos escopos de destinatário, o cmdlet **Enable-Mailbox** , que está incluído com esta função, também é escopo usando escopos de configuração do banco de dados. Escopos de configuração do banco de dados controlam que bancos de dados que o cmdlet pode criar novas caixas de correio em. O banco de dados onde você deseja criar uma caixa de correio deve estar dentro do escopo de banco de dados. Isso se aplica tanto ao especificar um banco de dados usando o parâmetro *Database* no cmdlet **Enable-Mailbox** ou se você permitir a distribuição de caixa de correio automática selecionar o banco de dados para você. Para obter mais informações, consulte [Noções básicas sobre escopos da função de gerenciamento](understanding-management-role-scopes-exchange-2013-help.md).
