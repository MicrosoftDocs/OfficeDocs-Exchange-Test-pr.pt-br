---
title: 'Função de criação de destinatário do email: Exchange 2013 Help'
TOCTitle: Função de criação de destinatário do email
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/pt-br/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50486045
ms.date: 05/22/2018
...
Essa função não permite ativar o email de pastas públicas. Para ativar o email de pastas públicas, a função `Mail Enabled Public Folders` deve ser usada. Para obter mais informações, consulte [Função de pastas públicas habilitadas para email](mail-enabled-public-folders-role-exchange-2013-help.md).

Se sua organização mantenha o controle de acesso com base em uma função (RBAC) dividir o modelo de permissões, onde a criação de destinatário é executada por um grupo diferente do que aqueles que executam o gerenciamento de destinatários, atribua a função `Mail Recipient Creation` ao grupo de funções de gerenciamento que realiza a criação de destinatário e a função `Mail Recipients` ao grupo de funções que está executando o gerenciamento de destinatários.
...
Para obter mais informações sobre o RBAC e Active Directory dividir permissões, consulte [Compreendendo as permissões de divisão](understanding-split-permissions-exchange-2013-help.md).
...
## Considerações adicionais de escopo

Além dos escopos de destinatário, o cmdlet **New-Mailbox** , que está incluído com esta função, também é escopo usando escopos de configuração do banco de dados. Escopos de configuração do banco de dados controlam que bancos de dados que o cmdlet pode criar novas caixas de correio em. O banco de dados onde você deseja criar uma caixa de correio deve estar dentro do escopo de banco de dados. Esta condição se aplica quando você especificar um banco de dados usando o parâmetro *Database* no cmdlet **New-Mailbox** ou se você permitir a distribuição de caixa de correio automática selecionar o banco de dados para você. Para obter mais informações, consulte [Noções básicas sobre escopos da função de gerenciamento](understanding-management-role-scopes-exchange-2013-help.md).
